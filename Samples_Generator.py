...
pics_path = r"D:\Aaron\liewei tech\Datasets\CelebA\Img\img_celeba.7z\img_celeba\img_celeba"
targets_path = r"D:\Aaron\liewei tech\Datasets\CelebA\Anno\list_bbox_celeba.txt"
landmarks_path = r"D:\Aaron\liewei tech\Datasets\CelebA\Anno\list_landmarks_celeba.txt"
save_path = r"D:\Aaron\liewei tech\Datasets\Dataset_MTCNN_Study\dataset_landmarkers"
...
def generator(pics_size, stop_value, isLandmarks=False):
    ...
    try:
        ...
        for i, line in enumerate(open(targets_path)):
            if i < 2:
                continue
            x1, y1, x2, y2 = float(line.split()[1].strip()), float(line.split()[2].strip()), float(line.split()[1].strip()) + \
                             float(line.split()[3].strip()), float(line.split()[2].strip()) + 0.9*float(line.split()[4].strip())  # 原图框四个坐标

            w, h = float(x2 - x1), int(y2 - y1)  # 原图框长宽
            center_axes = [float(x1+w/2), float(y1+h/2)]  # 原图中心点坐标

            landmarks_line = open(landmarks_path)
            landmarks = landmarks_line.readlines()[i]
            px1 = float(landmarks.split()[1].strip())
            py1 = float(landmarks.split()[2].strip())
            px2 = float(landmarks.split()[3].strip())
            py2 = float(landmarks.split()[4].strip())
            px3 = float(landmarks.split()[5].strip())
            py3 = float(landmarks.split()[6].strip())
            px4 = float(landmarks.split()[7].strip())
            py4 = float(landmarks.split()[8].strip())
            px5 = float(landmarks.split()[9].strip())
            py5 = float(landmarks.split()[10].strip())
            ...
            for _ in range(5):
                ...
                _center_x = center_axes[0]+np.random.randint(int(-center_axes[0]*seed), int(center_axes[0]*seed))  # 中心坐标点滑动
                _center_y = center_axes[1]+np.random.randint(int(-center_axes[1]*seed), int(center_axes[1]*seed))  # 中心坐标点滑动
                ...
                # draw.rectangle([_x1, _y1, _x2, _y2], outline="blue")
                # img.show()
                # exit()
...
if __name__ == "__main__":
    # generator(12, 50000)
    # generator(24, 50000)
    generator(48, 50000, isLandmarks=True)
